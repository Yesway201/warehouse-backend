 ...
 * @param {string} token - The API token to mask
...
export function loadSettings() {
  try {
    ...
    if (!fileExists) {
      console.log(`[SmartsheetSettings] READ path=${SETTINGS_FILE} fileExists=false keysPresent=false`);
      return {
        apiToken: null,
        sheetId: null,
        mappings: [],
    ...
    console.log(`[SmartsheetSettings] READ path=${SETTINGS_FILE} fileExists=true keysPresent=${keysPresent}`);
    
    return settings;
  } catch (error) {
    console.error('[SettingsStore] Error loading settings:', error);
    console.log(`[SmartsheetSettings] READ path=${SETTINGS_FILE} fileExists=error keysPresent=false`);
    return {
      apiToken: null,
      sheetId: null,
      mappings: [],
 ...
 * @param {Object} settings - Settings object with apiToken, sheetId, and mappings
...
export function saveSettings(settings) {
  try {
    ...
    console.log(`[SmartsheetSettings] WRITE path=${SETTINGS_FILE} keysPresent=${keysPresent}`);
    
    return true;
  } catch (error) {
    console.error('[SettingsStore] Error saving settings:', error);
    console.log(`[SmartsheetSettings] WRITE path=${SETTINGS_FILE} keysPresent=error`);
    return false;
  }
}
