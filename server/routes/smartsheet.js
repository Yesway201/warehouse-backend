...
/**
 * PATCH /api/smartsheet/mappings
 * Update only column mappings without requiring API token
 * NEW ENDPOINT for saving mappings independently
 */
router.patch('/mappings', async (req, res) => {
  console.log('[Smartsheet] PATCH /mappings');
  try {
    ...
    if (!existingSettings.apiToken || !existingSettings.sheetId) {
      return res.status(400).json({
        success: false,
        error: 'Smartsheet credentials must be configured before saving mappings'
      });
    }

    // Update only the mappings, keep existing credentials
    const saved = saveSettings({
      apiToken: existingSettings.apiToken,
      sheetId: existingSettings.sheetId,
      mappings: mappings
    });
    ...
    if (!saved) {
      return res.status(500).json({
        success: false,
        error: 'Failed to save mappings to server storage'
      });
    }
    ...
    console.error('[Smartsheet] Failed to save mappings:', error);
    return res.status(500).json({
      success: false,
...
router.post('/sync-deliveries', async (req, res) => {
  ...
  try {
    ...
    const USING_COLUMNS = ['Customer Name', 'PO # / Container #', 'Status', 'Done', 'ASN'];
    ...
    const asnColumnId = columnMap['ASN'];
    ...
    sheetData.rows.forEach((row, index) => {
      ...
      const asnCell = row.cells.find(c => c.columnId === asnColumnId);
      const asnRaw = asnCell?.value === true;
      ...
      const delivery = {
        ...
        asn: asnRaw,  // ASN checkbox: true = customer sent packing slip
...
router.post('/update-delivery', async (req, res) => {
  ...
  try {
    ...
    console.log('[Smartsheet] Column mappings:', settings.mappings);
    console.log('[Smartsheet] Field to Column ID map:', fieldToColumnId);
    console.log('[Smartsheet] Updates to apply:', updates);
    ...
    if (updates.referenceNumber !== undefined) {
      ...
      if (columnId) {
        ...
        console.log('[Smartsheet] WARNING: No column mapping found for referenceNumber');
      }
    }
    ...
    if (updates.extensivReceiptId !== undefined) {
      ...
      if (columnId) {
        ...
        console.log('[Smartsheet] WARNING: No column mapping found for extensivReceiptId');
      }
    }
    ...
    if (updates.status !== undefined) {
      ...
      if (columnId) {
        ...
        console.log(`[Smartsheet] Adding status update: columnId=${columnId}, value="${updates.status}"`);
      } else {
        console.log('[Smartsheet] WARNING: No column mapping found for status');
      }
    }

    // Add door support
    if (updates.door !== undefined) {
      const columnId = fieldToColumnId['door'];
      if (columnId) {
        cells.push({ columnId, value: updates.door });
        console.log(`[Smartsheet] Adding door update: columnId=${columnId}, value="${updates.door}"`);
      } else {
        console.log('[Smartsheet] WARNING: No column mapping found for door');
      }
    }

    // Add ASN checkbox support (Mark Received → check ASN in Smartsheet)
    if (updates.asn !== undefined) {
      // Try mapping first, then fallback to direct column title lookup
      const columnId = fieldToColumnId['asn'] || columnMap['ASN'];
      if (columnId) {
        cells.push({ columnId, value: updates.asn });
        console.log(`[Smartsheet] Adding ASN update: columnId=${columnId}, value=${updates.asn}, source=${fieldToColumnId['asn'] ? 'mapping' : 'direct-column-lookup'}`);
      } else {
        console.log('[Smartsheet] WARNING: No column mapping found for asn AND no "ASN" column found in sheet');
      }
    }

    console.log('[Smartsheet] Cells to update:', JSON.stringify(cells, null, 2));
    ...
    if (cells.length === 0) {
      return res.json({
        ...
        diagnostics: {
          ...
          fieldToColumnId,
          updates,
          reason: 'No column mappings found for the requested fields'
        }
      });
    }

    // Prepare the update payload - FIX: Use Number() instead of parseInt()
    const updatePayload = [{
      id: Number(rowId),
      cells,
    }];

    console.log('[Smartsheet] Update payload:', JSON.stringify(updatePayload, null, 2));
    ...
    console.log(`[Smartsheet] Smartsheet API response status: ${updateResponse.status}`);
    console.log(`[Smartsheet] Smartsheet API response body: ${responseText}`);
